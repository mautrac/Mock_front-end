...
const Group = (props) => {
  ...
  //get data after rendering (componentDidMount)
  //
  useEffect(() => {
    const getAllGroup = async () => {
      ...
      //dispatch to update state in store
          ...
          validationSchema={
            Yup.object({
              ...
              totalMember: Yup.number()
                .min(10, 'Must be in range (10, 15) and integer')
                .max(15, 'Must be in range (10, 15) and integer')
...
function rowStyleFormat(row, rowIdx) {
  return { backgroundColor: rowIdx % 2 === 0 ? 'red' : '' };
}

const rowClasses = (row, rowIdx) => {
  
  return rowIdx % 2 === 0 ? 'text-red' : '';
};
